      ...
      {(() => {
        ...
        return (
          <div className="rounded-lg border overflow-hidden">
            ...
            {defHoldings.length > 0 && (
              <>
                ...
                {defHoldings.map((h) => {
                  ...
                  return (
                    <div key={h.symbol}
                      className="flex items-center px-4 py-2 text-sm hover:bg-muted/40 transition-colors border-b">
                      <span className={`${COL.symbol} font-mono font-semibold text-xs tracking-wider`}>{h.symbol}</span>
                      <span className={`${COL.name} text-xs text-muted-foreground truncate`}>{h.name}</span>
                      <span className={`${COL.shares} tabular-nums text-xs ${amtClass}`}>{fmt(h.shares, 4)}</span>
                      <span className={COL.price}>
            ...
            {/* 現金 & 儲蓄險（含 target_pct 編輯） */}
            {state.cash_accounts.map((c, idx) => {
              const twd = c.currency === 'USD' ? c.amount * fx : c.amount
              const shortName = c.bank.split(' ')[0]
              return (
                <div key={c.id}
                  className={`flex items-center px-4 py-2 text-sm hover:bg-muted/40 transition-colors ${idx < state.cash_accounts.length - 1 ? 'border-b' : ''}`}>
                  ...
                  <span className={`${COL.shares} tabular-nums text-xs text-muted-foreground ${amtClass}`}>
